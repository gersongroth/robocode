...
class RobotEditor(ScrolledText):
    ...
    def onChangeColour(self):
        ...
        result = colorchooser.askcolor(title="Choose the robot colour", color=(r,g,b))
        if result:
            # get rgb value
            rgb = str(result[0])
            if rgb != 'None':
...
def addRobotToTesterFile(robotname):
    Testerpure = open("components/TesterPURE.py")
    newlines = []
    ...
    testerfile = open("components/Tester.py", 'w')
    testerfile.writelines(newlines)

def addTesterToRobotFile(robotname):
    Testerpure = open("robots/"+robotname+".py")
    newlines = []
    for line in Testerpure:
    ...
    testerfile = open("robots/"+robotname+"TEST.py", 'w')
    testerfile.writelines(newlines)
