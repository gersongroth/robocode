...
from components.Classes import BlastC, DeadRobotC, RobotC
sys.path.append(sys.path[0] + "/robots")

#import Andrew_Cassius, MaxPayne, MaxPayneTEST, Michael_Tarquin, Neil_Samson, sampleStan, Stuart_VINCENT

#robotmodules = [Andrew_Cassius, MaxPayne, MaxPayneTEST, Michael_Tarquin, Neil_Samson, sampleStan, Stuart_VINCENT]
robotmodules = []
...
def Menu(screen,width,height,Arena):
    ...
    def GameOver(Robots):
        ...
        def MainMenu(screen):
            import components.setup as setup
            setup.ScreenSetUp()
            Exit(screen)
