...
def CreateABot(screen):
    import components.EditorMenu as EditorMenu
    EditorMenu.ScreenSetUp()
    Exit(screen);

def TeamEditor(screen):
    import components.TeamEditorMenu as TeamEditorMenu
...
def StartGame(screen,BattleType,Arena,Robots):
    ...
    import components.gamefile as gamefile
...
def addRobotsToGameFile(robotnames,robotfiles, Arena, gamefile):
    gamefilepure = open("components/"+gamefile+"PURE.py")
    newlines = []
    ...
    gamefile = open("components/"+gamefile+".py", 'w')
